...
events = ['Dinner Party', 'Birthday Party', 'Marriage', 'PhD Party', 'Party Party', 'Oktoberfest', 'Christmas Party',
          'Carnival']
# ----------------------------------------------------------------------------------------------------------------------
...
if 'events_sortable_by_date' not in st.session_state:
    ...
    st.session_state['events_sortable_by_date'] = st.multiselect("events sortable by date", events,
                                                                 default=st.session_state['events_sortable_by_date'])
...
def sort_by_buttons(event_list: list):
    ...
    if len(event_list) > 0:
        for index_2, item_2 in enumerate(event_list):
            ...
            col1_sortable_by_buttons, col2_sortable_by_buttons, col3_sortable_by_buttons, col4_sortable_by_buttons = container_sortable_by_buttons.columns(
                4)
            with col1_sortable_by_buttons:
                st.write(item_2)
            with col2_sortable_by_buttons:
                number_of_guests_2 = st.number_input('Number of Guests', min_value=0, max_value=100, step=1,
                                                     key=f'{item_2}_button')
